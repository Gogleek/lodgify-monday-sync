...
@app.get("/health")
def health():
    return jsonify({"ok": True, "service": "lodgify-monday", "board_id": monday.board_id}), 200
...
def lodgify_sync_all():
    ...
    try:
        ...
        log.info("Fetched %d booking(s) from Lodgify [limit=%s, skip=%s]", len(bookings), limit, skip)
        results = []
        for bk in bookings:
            try:
...
def map_booking_to_monday(bk: dict) -> dict:
    ...
    status_label = DROPDOWN_LABELS.get("status", {}).get(status, DROPDOWN_LABELS["status"].get("default", "Pending"))

    check_in = bk.get("check_in") or bk.get("arrival")
    check_out = bk.get("check_out") or bk.get("departure")
    total_price = bk.get("total") or bk.get("total_amount") or 0
    currency = bk.get("currency") or bk.get("currency_code") or "GBP"
